...
async def follow_notification(id_title: int, message: types.Message):
    ...
    if not await DataBase.find_one('chat_id', message.chat.id, 'user_follows'):
        await DataBase.insert_into_collection('user_follows', {'chat_id': message.chat.id, 'animes': []})
    ...
    # update user follows
    ls.append(id_title)
    await DataBase.update_one('user_follows', 'chat_id', message.chat.id, {'animes': ls})
...
async def unfollow_notification(id_title: int, message: types.Message):
    ...
    await DataBase.update_one('user_follows', 'chat_id', message.chat.id, {'animes': animes})
...
async def send_notification(event: TitleEpisode):
    ...
    # Get users follows
    all_users = await DataBase.find('user_follows')
        ...
        if event.title.id in [int(i) for i in user['animes']]:
            await dp.bot.send_photo(user['chat_id'], f"{event.title.posters.small.full_url}",
                                    caption=f"<i>Вышла Новая Серия</i>"
                                            ...
                                            f"<b>Серия {event.episode.episode}</b>\n\n"
