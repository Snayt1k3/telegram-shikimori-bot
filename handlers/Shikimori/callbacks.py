...
async def callback_for_user_list(call: types.CallbackQuery):
    ...
    if datas[3] == 'next':
        await edit_reply_markup_user_lists(call.message, datas[0], "+", int(datas[2]))

    elif datas[3] == 'prev':
        await edit_reply_markup_user_lists(call.message, datas[0], "-", int(datas[2]))

    else:
        kb = cr_kb_by_collection(datas[0], datas[1], int(datas[2]))
...
async def anime_edit(call: types.CallbackQuery):
    ...
    if datas[3] == 'delete':
        ...
        kb.add(InlineKeyboardButton('<< Back', callback_data=f"back.{datas[1]}.{datas[0]}.{datas[2]}.update_score"))
...
async def update_score(call: types.CallbackQuery):
    # "action.target_id.coll.page.update_score"
    datas = call.data.split('.')
