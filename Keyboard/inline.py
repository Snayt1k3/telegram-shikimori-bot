...
def cr_kb_search_edit(target_id):
    ...
    planned = InlineKeyboardButton("📝 Into Planned list", callback_data=f"anime_search_edit.{target_id}.planned")
    completed = InlineKeyboardButton("☑ Into Completed", callback_data=f"anime_search_edit.{target_id}.completed")
...
def cr_kb_by_collection(coll, target_id, page):
    ...
    if coll == 'anime_watching':
        ...
        kb.add(
            InlineKeyboardButton("✔️ Into Completed", callback_data=f'{coll}.{target_id}.complete.anime_edit'),
